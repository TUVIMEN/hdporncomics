...
kwarg( ```python user_agent: str = "Mozilla/5.0 (X11; Linux x86_64; rv:135.0) Gecko/20100101 Firefox/135.0"``` ) - user agent

kwarg( ```python timeout: int = 30``` ) - timeout

kwarg( ```python retries: int = 3``` ) - number of retries in case of non fatal failure

kwarg( ```python retry_wait: float = 60``` ) - waiting time before retrying

kwarg( ```python wait: float = 0``` ) - waiting time in seconds in between requests

kwarg( ```python wait_random: int = 0``` ) - random waiting time in milliseconds in between requests

kwarg( ```python logger: Optional[TextIO] = None``` ) - file to which requests log will be written, e.g. sys.stderr or sys.stdout

fingerprint is generated at initialization so method( login ) is not needed unless you want to change it or log in.

Its recommended to set kwarg( ```python wait``` ).
...
### get_comic_fname(```python url: str```) -> ```python str```

Makes file name based on arg( `url` )

returns( `file name` )

### comic_link_from_id(```python c_id: int```) -> ```python str```

Creates url to comic from its id arg( `c_id` )

returns ( url to comic )

### comic_thumb(```python upload: str```) -> ```python str```

Converts url of image to its thumbnail version

returns( `url to thumbnail` )

### view(```python self, c_id: int, add: bool = True```) -> ```python bool | dict```
...
### get_comments(```python self, c_id: int, page: int = 1, top: bool = False```) -> ```python Generator```
...
### get_comic(```python self, url: str, c_id: int = 0, comments: int = 0, likes: bool = True```) -> ```python dict```
...
### get_manhwa_chapter(```python self, url: str, comments: int = 0```) -> ```python dict```
...
### get_manhwa(```python self, url: str, c_id: int = 0, comments: int = 0, likes: bool = True```) -> ```python dict```
...
### get_comic_file(```python self, url: str, c_id: int = 0, comments: int = 0, likes: bool = True```) -> ```python Optional[str]```

Downloads comic into a file passing all arguments to method( `get_comic` ).

returns( `file name or None if file already exists` )

### get_pages(```python self, url: str```) -> ```python Generator```
...
### comment_like(```python self, co_id: int, like: bool = True```) -> ```python bool```
...
### comment_delete(```python self, co_id: int```) -> ```python bool```
...
### favorite(```python self, c_id: int, add: bool = True```) -> ```python bool```
...
### comment(```python self, c_id: int, text: str, parent: int = 0```) -> ```python bool```
...
### comment_edit(```python self, co_id: int, text: str```) -> ```python bool```
...
### get_manhwa_authors_list(```python self```) -> ```python dict```
...
### get_manhwa_genres_list(```python self```) -> ```python dict```
...
### get_gay_genres_list(```python self```) -> ```python dict```
...
### get_gay_groups_list(```python self```) -> ```python dict```
...
### get_gay_languages_list(```python self```) -> ```python dict```
...
### get_gay_sections_list(```python self```) -> ```python dict```
...
### get_comics_list_url(```python self, url: str```) -> ```python Generator```
...
### get_comics_list(```python self, ctype: str, page: int = 1, sort: str = "", search: str = ""```) -> ```python Generator```
...
### get_terms(```python self, ctype: str```) -> ```python list```
...
### subscribe(```python self, term_id: int, add: bool = True```) -> ```python bool```
...
### get_dashboard_stats(```python self```) -> ```python dict```
...
### get_history(```python self```) -> ```python Generator```
...
### get_liked(```python self```) -> ```python Generator```
...
### get_favorites(```python self```) -> ```python Generator```
...
### get_subscriptions(```python self```) -> ```python list[dict]```
...
### get_user_comments(```python self```) -> ```python Generator```
...
### get_notifications(```python self```) -> ```python Generator```
...
### notifications_clean(```python self```) -> ```python bool```

Cleans all notifications for logged in user

returns( `True for success` )

### get_user(```python self, url: str```) -> ```python dict```
...
### search(```python self, search: str```) -> ```python Generator```

Searches for arg( `search` ) in titles of comics

returns( `initialized method( get_pages` ) )

### guess(```python self, url: str```) -> ```python Optional[Callable]```
